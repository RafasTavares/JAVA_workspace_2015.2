...
public class ArvoreBinariaBuscaNova<T extends Comparable<T>> {
	 ...
	 * 
	 * if (this.pai == null) { this.pai = noAux; return true; } else { if
	 * (this.pai.getElemento().compareTo(valor) >= 0) { if
	 * (noAux.getFilhoEsquerdo() == null) { this.pai.setFilhoEsquerdo(noAux);
	 * noAux.setPai(this.pai); return true; } else { return
	 * this.Inserir(noAux.getFilhoEsquerdo().getElemento()); } } else { if
	 * (noAux.getFilhoDireito() == null) { this.pai.setFilhoDireito(noAux);
	 * noAux.setPai(this.pai); return true; } else { return
	 * this.Inserir(noAux.getFilhoDireito().getElemento()); } }
	 * 
	 * }
	 * 
	 * }
	 * 
	 * public boolean remover(Comparable valor){ No noAux = new No(valor);
	 * if(noAux.getPai() == null){ noAux = null; return false; }else
	 * if(noAux.getFilhoEsquerdo() == null && noAux.getFilhoDireito() == null){
	 * 
	 * if(noAux.getPai().compareTo(noAux.getElemento()) > 0){
	 * noAux.getPai().setFilhoEsquerdo(null); noAux.setPai(null); }else{
	 * noAux.getPai().setFilhoDireito(null); noAux.setPai(null); } } else
	 * if(noAux.getFilhoEsquerdo() != null && noAux.getFilhoEsquerdo() == null
	 * || noAux.getFilhoEsquerdo() == null && noAux.getFilhoDireito() != null){
	 * if(noAux.getFilhoEsquerdo() != null && noAux.getFilhoDireito() == null){
	 * if(this.pai.compareTo(noAux.getElemento()) < 0){
	 * noAux.getPai().setFilhoDireito(noAux.getFilhoEsquerdo()); }else{
	 * noAux.getPai().setFilhoEsquerdo(noAux.getFilhoDireito()); }
	 * noAux.getFilhoEsquerdo().setPai(noAux.getPai()); }else
	 * if(noAux.getFilhoDireito() != null && noAux.getFilhoEsquerdo() == null){
	 * if(this.pai.compareTo(noAux.getElemento()) < 0){
	 * noAux.getPai().setFilhoDireito(noAux.getFilhoDireito()); }else{
	 * noAux.getPai().setFilhoEsquerdo(noAux.getFilhoEsquerdo()); }
	 * noAux.getFilhoDireito().setPai(noAux.getPai()); } }else{ } return true; }
	 * 
	 * public boolean inserir2(Comparable Valor) {
	 * 
	 * No NovoNo = new No(Valor);
	 * 
	 * if (this.pai == null) { this.pai = NovoNo; System.out.println(
	 * "this.pai: " + this.pai.getElemento()); return true;
	 * 
	 * } else { No NoAtual = this.pai;
	 * 
	 * if (NoAtual.getElemento().compareTo(NovoNo.getElemento()) == -1) {
	 * 
	 * if (NovoNo.getFilhoDireito() == null) { this.pai.setFilhoDireito(NovoNo);
	 * NovoNo.setPai(this.pai); return true;
	 * 
	 * } else { Valor = NovoNo.getFilhoDireito().getElemento();
	 * this.inserir2(Valor); }
	 * 
	 * } else {
	 * 
	 * if (NovoNo.getFilhoEsquerdo() == null) {
	 * this.pai.setFilhoEsquerdo(NovoNo); NovoNo.setPai(this.pai); return true;
	 * 
	 * } else { Valor = NovoNo.getFilhoEsquerdo().getElemento();
	 * this.inserir2(Valor); }
	 * 
	 * }
	 * 
	 ...
	 * 
	 * ArrayList<Comparable> lista = new ArrayList<Comparable>(); Queue<No>
	 * listaAtual; listaAtual = new ArrayDeque<No>();
	 * 
	 * listaAtual.add(this.pai); No noAux;
	 * 
	 * while (listaAtual.size() > 0) {
	 * 
	 * noAux = listaAtual.poll();
	 * 
	 * if (noAux != null) { lista.add(noAux.getElemento());
	 * 
	...
	private ArrayList<T> RetornaLista(NoNovo<T> no, ArrayList<T> lista) { // Função
																			// reculsivo
																			// do
																			// RetornarLista
